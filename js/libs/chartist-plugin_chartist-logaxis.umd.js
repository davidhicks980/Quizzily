(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(require('chartist')) :
    typeof define === 'function' && define.amd ? define(['chartist'], factory) :
    (global = global || self, factory(global.Chartist));
}(this, function (Chartist) { 'use strict';
    ...
    function AutoScaleAxis(axisUnit, data, chartRect, options) {
        ...
        if (this.scale.type === 'log') {
            ...
            this.bounds = getLogAxisBounds(chartRect[axisUnit.rectEnd] - chartRect[axisUnit.rectStart], highLow, options.scaleMinSpace || 20, options.onlyInteger, this.scale.base);
        }
        else {
            // Usually we calculate highLow based on the data but this can be
            // overriden by a highLow object in the options
            var highLow = options.highLow || globalChartist.getHighLow(data, options, axisUnit.pos);
            this.bounds = globalChartist.getBounds(chartRect[axisUnit.rectEnd] - chartRect[axisUnit.rectStart], highLow, options.scaleMinSpace || 20, options.onlyInteger);
        }
        this.range = {
            min: this.bounds.min,
            max: this.bounds.max
        };
        globalChartist.AutoScaleAxis["super"].constructor.call(this, axisUnit, chartRect, this.bounds.values, options);
    }
    function createGridAndLabels(gridGroup, labelGroup, useForeignObject, chartOptions, eventEmitter) {
        var _this = this;
        var axisOptions = chartOptions['axis' + this.units.pos.toUpperCase()];
        globalChartist.AutoScaleAxis["super"].createGridAndLabels.call(this, gridGroup, labelGroup, useForeignObject, chartOptions, eventEmitter);
            ...
            this.ticks.forEach(function (tick, index) {
                ...
                if (nextTick) {
                    ...
                    for (var i = 1; i < sections; ++i) {
                        ...
                        globalChartist.createGrid(projectedValue, index, _this, _this.gridOffset, _this.chartRect[_this.counterUnits.len](), gridGroup, [
    ...
    function getLogAxisBounds(axisLength, highLow, scaleMinSpace, onlyInteger, logBase) {
        var bounds = globalChartist.getBounds(axisLength, highLow, scaleMinSpace, onlyInteger);
    ...
    function getLogAxisHighLow(data, options, dimension) {
        var highLow = options.highLow || globalChartist.getHighLow(data, options, dimension);
        if (data.length === 0) {
            highLow.low = 1;
            highLow.high = 1000;
        }
